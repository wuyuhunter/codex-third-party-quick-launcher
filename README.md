...
`v0.5.5` 起，本项目把国产模型适配作为一个稳定发布基准继续维护，并补上 provider/KEY 中文命名冲突、“新增服务”误覆盖旧服务、Windows PowerShell 5.1 启动兼容问题，以及 DeepSeek 等 Chat Completions 通道的 Codex 工具调用适配：

- 不依赖 `~/.codex/config.toml` 保存模型服务；启动时通过本次进程参数注入 provider。
- 未在本启动器配置页保存的全局 `OPENAI_API_KEY`、`~/.codex/auth.json` 和旧启动器选择变量不会被自动当作 KEY 使用。
- 中文或近似名称的新服务、新 KEY 会自动分配唯一内部 id，不会再因为内部 id 冲突互相覆盖。
- 自定义 OpenAI-compatible 地址如果只填站点根路径，启动器会自动补齐常见的 `/v1`，避免请求误打到 HTML 页面。
- 桌面快捷方式通过 Windows PowerShell 5.1 启动时不再因为脚本语法兼容问题直接退出。
- 支持“模型系列 -> 模型版本 -> 推理深度”的配置结构，聚合平台可以勾选多个模型系列。
- 对 Qwen、DeepSeek、GLM、Kimi、MiniMax 提供本机 adapter 转译路径，兼容新版 Codex CLI 的 Responses 调用方式。
- Chat Completions 本机 adapter 支持 Codex `function_call` / 工具结果双向转换，DeepSeek 可驱动 Codex 执行只读工具流程。
- 可配置用户自定义 OpenAI-compatible Responses 通道，方便用户在本机对照排查 provider 和模型映射问题。
...
`核心安装` 会自动处理两类常见 Windows 空白机问题：

- 缺少 `vcruntime140.dll` / `msvcp140.dll`：安装 Microsoft Visual C++ Redistributable x64。
- `npm` 已安装但 PowerShell 中无法执行：把当前用户执行策略设置为 `RemoteSigned`。
...
重编 Windows exe 优先使用 Win10 / Win11 自带的 .NET Framework 编译器 `csc.exe`，通常不需要安装 .NET 8 SDK。运行：
